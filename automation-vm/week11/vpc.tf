...
resource "aws_internet_gateway" "week11-igw" {
	vpc_id = aws_vpc.week11-vpc.id
...
resource "aws_default_route_table" "week11-rt" {
  default_route_table_id = aws_vpc.week11-vpc.default_route_table_id
  route = [
    {
      cidr_block = "0.0.0.0/0"
      gateway_id = aws_internet_gateway.week11-igw.id
